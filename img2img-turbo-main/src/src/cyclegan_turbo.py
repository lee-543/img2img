...
def initialize_unet(rank, return_lora_module_names=False):
    unet = UNet2DConditionModel.from_pretrained("../sd-turbo", subfolder="unet")
...
def initialize_vae(rank=4, return_lora_module_names=False):
    vae = AutoencoderKL.from_pretrained("../sd-turbo", subfolder="vae")
...
class CycleGAN_Turbo(torch.nn.Module):
    def __init__(self, pretrained_name=None, pretrained_path=None, ckpt_folder="checkpoints", lora_rank_unet=8, lora_rank_vae=4):
        super().__init__()
        self.tokenizer = AutoTokenizer.from_pretrained("../sd-turbo", subfolder="tokenizer")
        self.text_encoder = CLIPTextModel.from_pretrained("../sd-turbo", subfolder="text_encoder").cuda()
